...
if (!empty($_SESSION["user"])) {
...
}else if (!empty($_COOKIE["login_usuario"]) && !empty($_COOKIE["random_password"]) && !empty($_COOKIE["random_selector"])) {// Verificar si existen cookies de autenticación
    ...
    if (!empty($userToken[0]["id"]) && $isPasswordVerified && $isSelectorVerified && $isExpiryDateVerified) {
        ...
        // Acceder al ID del usuario desde la cookie
        if (!empty($_COOKIE["user_id"])) {
            // Desencriptar el ID del usuario
            $decrypted_user_id = openssl_decrypt($_COOKIE["user_id"], 'AES-128-ECB', $userToken[0]["selector_hash"]);
            // Utilizar el ID del usuario desencriptado
            $_SESSION['user'] =  getIdUser($conexion, $decrypted_user_id);
            $_SESSION['botSelected'] = 0;
        } else {
            //No se encontró la cookie del ID del usuario
